...
class Common {
    ...
    class func isValidAPIKey() -> Bool {
        if WakatoonSDKData.shared.API_KEY.trimmingCharacters(in: .whitespacesAndNewlines).count > 0 {
            ...
            WakatoonSDKData.shared.delegate.invalidAPIKey()
            return false
        }
    }
...
extension Common {
    ...
    class func downloadEpisodeFromURL(_ videoLink: String, isFor: APIManager.VideoLabel, loopTimecode: String?) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now()) {
            ...
            DownloadManager(name: UUID().uuidString, url: videoURL).download { percent in
                ...
                switch isFor {
                    case .DETECTED_LOOP:
                        VideoCatchModel(savedURLString: savedVideoString).saveCatchVideo(.DETECTED_LOOP_DATA, loopTimecode: loopTimecode)
                        break
                    case .EPISODE:
                        VideoCatchModel(savedURLString: savedVideoString).saveCatchVideo(.EPISODE_DATA, loopTimecode: loopTimecode)
                        break
                    case .INTRO:
                        VideoCatchModel(savedURLString: savedVideoString).saveIntroCatchVideo(loopTimecode: loopTimecode)
                        break
                    case .DETECTED:
                        VideoCatchModel(savedURLString: savedVideoString).saveCatchVideo(.DETECTED_DATA, loopTimecode: loopTimecode)
                }
            }.onError { (error) in
